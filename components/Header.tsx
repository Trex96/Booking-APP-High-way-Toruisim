...
export default function Header() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Automatically redirect to home page when search is cleared
    if (!value.trim()) {
      router.push("/");
    } else {
          ...
          >
            <Link href="/" className="flex items-center gap-1 xs:gap-2">
              ...
              <div className="flex flex-col leading-none">
                <span className="text-base xs:text-lg font-bold">Highway</span>
                <span className="text-base xs:text-lg font-bold">delite</span>
              </div>
            </Link>
          </motion.div>
